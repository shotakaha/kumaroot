...
** Emacs
   ...
   最初に断っておくと、特にEmacs信者というつもりはないのです。
   修士でプログラミングを始めた時に、最初に使ったのがEmacsだったというだけの理由です。
   Vimも基本操作はできるようにしといた方がよいです。
...
*** ページ操作
    ...
    | C-p        | k      | k       | 前の行                     |
    | C-v        | SPC    | C-f     | １ページ進む               |
    | M-v        | S-SPC  | C-b     | １ページ戻る               |
    | M-<        | g      | gg      | ファイルの先頭             |
    | M->        | G      | G       | ファイルの最後             |
    | M-g g 数値 | :数値  | 数値 G  | 指定した数値の行へジャンプ |
    |            | d      |         | 半ページ進む               |
    |            | u      |         | 半ページ戻る               |
...
*** YaTeXを使おう
    ...
    最初に断っておくと、特にEmacs信者というつもりはないのです。修士に
    入って使い始めたのがVimではなくEmacsだったので、そのままという感じ
    です。あ、MacOSXのソフト（Cocoa？）が同じキーバインドで操作できたっ
    てのもあるかも。
