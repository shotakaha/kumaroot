...
This project uses a **calendar-based semantic versioning** scheme (YYYY.MM.PATCH). See [Version Management Strategy](#version-management-strategy) for when to bump each part.
...
6. Commit per [Commit types used in this project](#git-workflow) (new page → `feat`)
...
11. Commit per [Commit types used in this project](#git-workflow) (new page → `feat`)
...
8. Commit per [Commit types used in this project](#git-workflow) (new page → `feat`)
...
This is a **documentation-first project**: the CHANGELOG (auto-generated by commitizen) only surfaces `feat`, `fix`, and `refactor` commits under their respective headings. Use those three for changes to published content under `docs/source/`; use anything else for project-internal files that don't need a CHANGELOG entry.

| Type | When | In CHANGELOG? |
| --- | --- | --- |
| `feat()` | New page under `docs/source/` | Yes |
| `fix()` | Updates/corrections to an existing page under `docs/source/` (primary type here) | Yes |
| `refactor()` | Reorganizing existing content under `docs/source/` without changing its meaning | Yes |
| `docs()` | Meta documentation files: `AGENTS.md`, `CLAUDE.md`, `README.md` | No |
| `chore()` | Config/tooling files: `.pre-commit-config.yaml`, `pyproject.toml`, `Taskfile.yml`, GitHub Actions | No |
| `bump:` (no scope, not a conventional-commit type) | Version bump message auto-generated by `task bump` | Creates the version heading itself |
...
- `docs(AGENTS): document commit type split`
...
- **PATCH**: After each `feat`/`fix`/`refactor` commit (see [Commit types](#git-workflow))
  - Use: `task bump:patch`
  - Example: `2025.11.6` → `2025.11.7`

- **MINOR**: When the calendar month changes（e.g., 2026.3.x → 2026.4.x）
  - Use: `task bump:minor`
  - Example: `2025.11.x` → `2025.12.0`
  - Usually done manually on the first commit of a new month

- **MAJOR**: When the calendar year changes（e.g., 2025.x.x → 2026.x.x）
  - Use: `task bump:major`
