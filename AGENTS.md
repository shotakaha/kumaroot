...
This is a **documentation-first project**: the CHANGELOG (auto-generated by commitizen) only surfaces `feat`, `fix`, and `refactor` commits under their respective headings — `docs`, `chore`, and other types are omitted from it. So changes to the published documentation content under `docs/source/` must use `feat`/`fix`/`refactor` to show up in the CHANGELOG; changes to project-internal files that don't need a CHANGELOG entry can use `docs`, `chore`, or another type.

- `feat()`: Creating a brand-new page under `docs/source/` (e.g. `docker/docker-example-fedora.md` did not exist before) — **shows up in CHANGELOG**
- `fix()`: Updates, additions, and corrections to existing pages under `docs/source/` (primary type for this documentation project) — **shows up in CHANGELOG**
- `refactor()`: Reorganizing existing content under `docs/source/` without changing its meaning (e.g. splitting/merging pages, simplifying structure) — **shows up in CHANGELOG**
- `docs()`: Changes to non-content, meta documentation files such as `AGENTS.md`, `CLAUDE.md`, `README.md` — not CHANGELOG-worthy
- `chore()`: Changes to configuration/tooling files that aren't documentation content, e.g. `.pre-commit-config.yaml`, `pyproject.toml`, `Taskfile.yml`, GitHub Actions workflows — not CHANGELOG-worthy
...
- `refactor(docker-example-wordpress): simplify to unified template structure`
- `docs(AGENTS): document feat()/fix()/refactor()/docs()/chore() split`
- `chore(pre-commit): update hook versions`
- `bump: version 2025.11.6 → 2025.11.7`

**Note:** Use `feat()` only when a new page file is created for the first time under `docs/source/`. Use `fix()` for other changes to existing pages under `docs/source/` (content additions, updates, improvements). Use `refactor()` when reorganizing existing content under `docs/source/` without changing its meaning. Use `docs()` for meta documentation files like `AGENTS.md`, `CLAUDE.md`, and `README.md`, and `chore()` for configuration/tooling files — neither needs to appear in the CHANGELOG.
