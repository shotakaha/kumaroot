...
:::{note}
テーマは配列で指定できるので、複数のテーマを設定できます。
先頭のテーマが優先されます。
:::

:::{caution}
`theme`キーを変更すれば、テーマを切り替えできます。
ただし、テーマごとにテンプレートの構造が異なるため、
レイアウトが崩れたり、ページが表示されなくなったりすることがあります。
テーマを切り替える場合は、テーマのドキュメントをよく読んで、必要に応じてテンプレートを修正してください。
:::
