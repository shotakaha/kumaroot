...
``uv``自体がPythonのバージョン管理機能を持っており、
``uv python install``でPython本体をインストールできます。
Homebrewや``pyenv``を使わなくても、``uv``ひとつで
Python本体のインストールからパッケージ管理・仮想環境の作成までを完結できます。

```console
// バージョンを指定してプロジェクト用の仮想環境を作成
$ uv venv --python 3.12
```

``uv venv --python``で、指定したバージョンのPythonを使った仮想環境を作成できます。
バージョンが未インストールの場合は自動でダウンロードされます。

:::{seealso}

詳細は[](./python-uv.md)に整理しました。
