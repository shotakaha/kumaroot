...
`glob`は指定したディレクトリの直下だけを検索します。
サブディレクトリも含めて再帰的に検索したい場合は`rglob`を使います
（`glob("**/*.csv")`と書いても同じ結果になります）。
...
## パスを確認したい（`exists` / `is_file` / `is_dir`）
...
`exists`でパスが存在するかどうかを確認できます。
存在することが確認できたら、`is_file`・`is_dir`でファイルかディレクトリかを判定できます。
存在しないパスに対して`is_file`・`is_dir`を呼んでも、`False`が返るだけでエラーにはなりません。
