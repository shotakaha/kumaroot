...
v11.1.0からQt6がサポートされたようですが、
v11.2.1ではうまくビルドできませんでした。
Qt5に対するサポートもまだ残っているので、
当面は`brew install qt@5`でQt5を指定するのがよさそうです。
...
v11.4.0のリリースノートを読むと、
CMakeオプションのデフォルトがQt6になったそうです。
また、Qt5を使う場合は`-DGEANT4_USE_QT_QT5=ON`が必要なようです。
