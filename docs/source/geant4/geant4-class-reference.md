...
検索窓はあいまい検索（fuzzy match）に対応していません。
長いクラス名を、前方から正しく入力しないと検索できないのは、ちょっと不便です。

また、実装の詳細は、ソースコード本体を手元に用意して、
コマンドライン検索（[fd](../command/command-fd.md)など）して
直接読んだほうが読みやすいかもしれません。
