...
フォーム本体に設定したフォーム送信トリガーでは、回答は`e.response`から取得します。
一方で、回答先スプレッドシートに設定したフォーム送信トリガーでは、
`e.values`や`e.namedValues`が使えます。
同じ「フォーム送信」でもトリガーを設定した場所によってイベントオブジェクトの構造が異なるため、
`Undefined`になった場合は、どちらのトリガーなのかを確認してください。
...
- `addListItem()`: プルダウン。
- `addMultipleChoiceItem()`: ラジオボタン。1つだけ回答可。
