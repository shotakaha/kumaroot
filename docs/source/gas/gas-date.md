...
Logger.log(now.toLocaleString("ja-JP", {timeZone: "Asia/Tokyo"}));
// 2024/11/11 10:37:03
```
...
| `yyyy` | 年（4桁） | Year | 2024（4桁） / 24（2桁） |
| `MM` | 月（2桁） | Month | 01 - 12 |
| `dd` | 日（2桁） | Number | 01 - 31 |
| `HH` | 時（2桁、24時間表記） | Number | 00 - 23 |
| `hh` | 時（2桁、12時間表記） | Number | 01 - 12 |
...
| `XXX` | タイムゾーン（ISO 8601） | ISO 8601 time zone | +09:00 |
