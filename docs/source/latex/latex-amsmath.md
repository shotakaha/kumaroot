...
`\mathrm{}`で数式用フォントの立体を適用できます。
SI（国際単位系）では、変数は斜体（italic）で、
数値や単位、関数名などは立体（upright）にするのが原則です。

:::{note}

さらにSIでは、数値と単位の間に空白を入れることも原則です。
LaTeXの数式環境では、スペースが無視されるため、
`\,`（thin space）を使ってスペースを入れる必要があります。
