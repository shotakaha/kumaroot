...
どのような内容にまとめるか、はやめに検討するとよいでしょう。
この段階で、LaTeXを触る必要はなく、WordやGoogleドキュメントなど、
日頃から使っているツールを使うとよいと思います。

また、研究室やグループの定例ミーティングなどでは、
スライド形式で報告することが多いかもしれませんが、
定期的に文章化しておくとよいと思います。

:::{note}

週に1回の定例ミーティングのスライドのおかげで、
修論に使う図やグラフの準備には（あまり）困りませんでした。
ただし、人間の記憶は頼りないもので、ところどころ議論の経緯が曖昧で、
文章を整理するのが大変なところもありました。

:::

## LaTeX環境を準備する

macOSやLinuxを使っているならば、HomebrewでMacTeXをインストールして、ローカル環境を構築してください。
...
`LuaLaTeX`と`jlreq`の場合、まず下記の内容で`main.tex`を作成します。
