...
日頃からログブックにきちんと記録したり、
グループの定例ミーティングで議論したりなどの積み重ねが大切です。
この段階では、LaTeXを使う必要はなく、
WordやPowerPointなど普段から使い慣れているツールを使えばよいです。

:::{hint}

実際に、週に1回の定例ミーティングで報告したスライドにとても助けられました。
ただし、文章として整理するのに苦労しました。
学会発表のあとや、研究のフェーズがひと区切りついたときに、
文章にして整理しておけばよかったかなと思います。

:::

## LaTeX環境を準備する

macOSやLinuxを使っているならば、
HomebrewでMacTeXもしくはTeX Liveをインストールして、
ローカル環境を構築できます。

最近では、OverleafやCloudLaTeXのようなオンラインサービスを使うのもありかもしれません。
ただし、無料プランだと容量の制限や、コンパイル時間の制限などがあります。
また、サービスがメンテに入ったり、ダウンすると使えなくなる可能性があります。

修論は提出〆切との闘いでもあります。
〆切間近に使えなくなるのはダメージが大きすぎるので、
ローカルに環境を構築することを個人的にはオススメします。
...
修論はページ数がそれなりに多くなります。
章ごとにディレクトリ／ファイルを分割して管理するとよいと思います。
...
共通ディレクトリに保存する形式と、
章ごとに作成したディレクトリの中に保存する形式があると思います。
前者が一般的な気がしますが、ここでは後者を選択しました。
それぞれの文書ソースと画像をまとめることができるのはよいと思います。

### 添削依頼用ファイルを作成する

指導教員や共同研究者など関係者に添削／校正を依頼する場合はPDF形式で送ります。
どの段階の内容を送ったのか、あとで確認できるように、別のディレクトリにコピーを保存しておくとよいです。
