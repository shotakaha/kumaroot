...
`numbering`オプションで、番号の表示方法を変更できます。
デフォルトは`numbering: "1."`です。
`numbering: "a."`でアルファベット表示にできます。
...
`separator`オプションで、単語と説明の区切りを変更できます。
デフォルトは、単語と説明の間にスペースが入るようになっています。
`separator: [: ]`でコロン（`:`）区切りにできます。
