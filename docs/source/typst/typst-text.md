...
```typst
ここはデフォルトのフォントで表示します。
ここは#text(font: "Harano Aji Mincho")[原ノ味明朝で表示]します。
ここは#text(weight: "bold")[太字で表示]します。
ここは#text(fill: luma(80%))[薄いグレーの文字で表示]します。
ここからまたデフォルトのフォントで表示
```

`text`関数で、本文中のテキストを部分的に変更できます。

:::{hint}

上記のサンプルはあくまでサンプルです。
実際には本文中で`text`関数を直接使うことはあまりないと思います。

後述する`#set text(...)`ルールで、ドキュメント全体のテキストを変更したり、
`#show`ルールで、見出しやコードブロックなどの特定の要素のテキストを変更したりすることが多いと思います。
...
`#set text(...)`で、ドキュメント全体のテキスト設定を変更できます。
`lang`、`font`、`size`、`weight`などのオプションをまとめて設定できます。
