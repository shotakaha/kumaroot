...
Typstでは `#let` キーワードを使って関数を定義します。引数の種類や制御フローなどを組み合わせることで、柔軟で再利用可能な関数が作成できます。

## 基本的な関数定義

もっともシンプルな関数は、引数を受け取って値を返します：
...
関数本体がシンプルな場合は、式のみで記述できます。複数の処理が必要な場合は、コードブロック `{...}` を使用します：
