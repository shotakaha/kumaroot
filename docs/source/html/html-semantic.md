...
[セマンティクス](https://developer.mozilla.org/ja/docs/Glossary/Semantics)とは、もともと「意味」を指す言葉です。
HTMLでは「そのタグが何を表すか」を意味します。

`<nav>`や`<article>`、`<h1>`は、「ナビゲーション」「記事」「見出し」という意味を持つタグです。
一方`<div>`と`<span>`は、意味を持たないただの箱です。
上の2つのサンプルはCSSを当てれば同じ見た目になりますが、
`<div class="nav">`はブラウザにとって「ただの箱」で、`<nav>`は「ここはナビゲーション」という意味まで伝わります。
セマンティクスを意識して書くとは、中身の意味に合ったタグを選ぶことです。

HTMLは人間だけでなく、ブラウザ・検索エンジン・スクリーンリーダーといった機械も読みます。
機械は見た目（CSS）を理解せず、タグの意味だけを頼りにします。
だから意味に合ったタグを使うと、スクリーンリーダーがランドマークや見出しでページ内を移動でき、
検索エンジンが構造を理解でき、CSSやJavaScriptも見た目用のクラス名に依存せず要素を指せます。
`<div>`だけで組んだHTMLは、見た目は作れても、これらの情報が伝わりません。

`<nav>`や`<article>`のようなタグは、はじめからHTMLにあったわけではありません。
2000年ごろにCSSが実用化されて「文書構造（HTML）と見た目（CSS）を分ける」考え方が広まった当時、
意味を持つタグは`<h1>`〜`<h6>`や`<ul>`くらいしかなく、`<div class="nav">`のようにクラス名で意味を補っていました。
その後、実際によく使われていたクラス名をもとに`<header>`、`<nav>`、`<main>`、`<article>`、`<section>`、`<aside>`、`<footer>`などが追加され、
2014年に標準（HTML5）として勧告されました。
いまでは`<div class="nav">`ではなく`<nav>`と書けば、意味まで伝わります。
