...
| タグ | 使う場面 |
| --- | --- |
| `div` | 意味を持たない箱。スタイルやスクリプトのために囲むだけ |
| `article` | ブログ記事、ニュース、コメント1件、商品カードなど、それ単体で完結するもの |
| `section` | 文書内の章・節。ランディングページのブロック、一覧のカテゴリ区分など |

まず見出しが付くかどうかで`div`かそれ以外かが決まります。
次に、そのまとまりが単体で完結するなら`article`、文書の一部なら`section`です。
