...
CSSは、詳細度の点数が同じスタイルがぶつかったとき、あとに書いたほうを採用します。
...
CSSは、値のうしろに`!important`と書くと、そのスタイルを最優先にします。
詳細度の点数や記述順を無視して勝ちます。
...
CSSは、一部のプロパティについて、親要素に指定した値を子要素が自動的に受け継ぎます。
このしくみを継承といいます。

上のサンプルでは、`body`に文字色とフォントを指定しています。
中にある`p`や`a`に何も指定しなくても、同じ文字色とフォントが適用されます。

継承されるのは、`color`、`font-family`、`font-size`、`line-height`など、文字まわりのプロパティが中心です。
...
CSSには、`currentColor`という特別なキーワードがあります。
これは「その要素にいま効いている`color`の値」を意味します。
