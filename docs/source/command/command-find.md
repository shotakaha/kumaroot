...
同じ条件は`-regex ".*\.\(jpg\|png\)$"`でも書けますが、`-regex`はファイル名ではなく検索パス全体にマッチする点に注意してください。
...
`-*time`系のオプションで、ファイルのタイムスタンプを使って検索できます。
`-mtime`は最終更新日、
`-atime`は最終アクセス日、
`-ctime`はステータス変更日を基準に検索できます。

基準は現在時刻で、
`-N`はN日**以内**、
`+N`はN日**以上前**にマッチします。

`-*min`系のオプションを使うと、日単位ではなく分単位で指定できます。
...
`-ls`で、検索結果を`ls -l`と同じ形式で表示できます。
パーミッションやサイズなどを確認したいときに便利です。

`-print0`で区切り文字を改行（`\n` / `0x0A` / `U+000A`）からヌル文字（`\0` / `0x00` / `U+0000`）に変更できます。
このオプションは、[xargs](./command-xargs.md)の`--null`（`-0`）オプションと組み合わせて使うことが多いです。

:::{hint}

`-print0`は、ファイル名が空白文字を含むなど、検索結果に半角スペース（`0x20` / `U+0020`）が含まれている場合に効果的です。
`xargs`は、引数を受け取るときに、改行と空白文字で区切ってしまいます。
そのため、空白文字を含むファイル名があると、そこで引数が分割されてしまい、意図しない動作となります。
`find -print0`でヌル文字区切りにすること、
そしてで、`xargs --null`でヌル文字を区切りとして受け取ることで、空白文字を含むファイル名でも正しく処理できるようになります。
...
検索した結果をパイプして、さらにテキスト処理できます。
そのとき[xargs](./command-xargs.md)に渡すことで、検索結果を任意のコマンドの引数として利用できます。

`-print0`オプションについては、前述の「検索結果の出力形式を変えたい」を参照してください。
