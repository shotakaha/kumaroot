...
この2つは`|`（パイプ）でいくつでも繋げられます（例：`.name | length`）。
...
デフォルト（`-r`なし）の結果をシェル変数に代入すると、ダブルクォートまで含めて代入されてしまいます。
ファイル名の組み立てや`if [ "$NAME" = "test" ]`のような文字列比較で、意図しない不一致やおかしなファイル名の原因になるので注意してください。
