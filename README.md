...
- **[KumaROOT - Read the Docs](https://kumaroot.readthedocs.io/ja/latest/)**（オススメ：最新かつもっとも読みやすい）
...
このセクションはこのドキュメントを一緒に**編集・拡張したい方**向けです。
また**ローカルでビルドしたい方**向けです。
...
リポジトリをクローンして、Python環境をセットアップし、`task docs:serve`を実行すればOKです。
...
`task code`は、常にプロジェクトルートでVS Codeを開くコマンドです。
また、uv環境を引き継ぐことができるので、そのままNotebookやターミナルを開いて作業できます。

`task docs:serve`は、ドキュメントをライブリロード付きでプレビューするコマンドです。
