...
個人ページでの公開は現在使用していません。
すべてのドキュメントは[Read the Docs](https://kumaroot.readthedocs.io/ja/latest/)で自動的に公開・更新されます。
...
`fix`または`feat`コミットを行った後に実行してください。
パッチバージョンを更新し、`CHANGELOG.md`が自動的に更新されます。
このコマンドをもっとも頻繁に使用します。
...
月が変わった時に1度だけ実行してください。例：2026年3月から4月への移行時。
マイナーバージョンを更新し、`CHANGELOG.md`が自動的に更新されます。

```bash
task bump:major
```

年が変わった時に1度だけ実行してください。例：2026年から2027年への移行時。
メジャーバージョンを更新し、`CHANGELOG.md`が自動的に更新されます。
